...
var of = {
    'head': {
        ...
        // -~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~
        /**
         * Ajoute une callback pour une clé au tableau {@link of.head._actions}
         *
        ...
        addAction: function(key,fn) {
            if( undefined === of.head._actions[key] ) {
                of.head._actions[key] = [fn];
            } else {
                of.head._actions[key].push(fn);
            }
            return of.head; // pour pouvoir enchaîner les addAction
        },
        // -~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~
        /**
        ...
        doAction: function (key) {
            var ret = null, args, acts = of.head._actions[key];
        ...
        // -~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~
        /**
         * Enlève une callback pour une clé au tableau {@link of.head._actions}.
         * Si plus de callback pour la clé, remove l'entrée.
         */
        removeAction: function (key, fn) {
            if( undefined !== of.head._actions[key] ) {
                var i = $.inArray(fn, of.head._actions[key]);
                if( -1 < i ) {
                    of.head._actions[key].splice(i,1);
                    if( 0 === of.head._actions[key].length ) {
                        delete of.head._actions[key];
                    }
                }
            }
        }
...
;(function(jQuery, of, undefined){
    if( undefined === of || undefined === of.head ) return;
    var $ = of.head.noConflict(jQuery); // Definition des variables internes "jQuery" et "$" pour le framework


    of.head.doc = document || window.document;
    of.head.$doc = $(of.head.doc);
    ...
    if (!Array.prototype.forEach) {
      ...
      Array.isArray = function(arg) {
        ...
        /**
     * https://raw.githubusercontent.com/WICG/IntersectionObserver/gh-pages/polyfill/intersection-observer.js
    ...
    })(window, function(exports, _srcDoc) {
        var idx, iframes;
    ...
    $.fn.extend( {
        ...
        /**
         * Addon jQuery.fn qui "Scanne" les éléments de l'objet jQuery.
         * Scanne signifie accrocher les comportements de composants OF qu'on y trouve.
         *
         * @param {boolean} [outerScan=false] true si l'objet jQuery est lui même à ofScanner, pas seulement son contenu.
         *          typiquement :
         *              $x.html(contenu).ofScan();
         *              $y.replaceWith($z); $z.ofScan(true);
         *
         * @return {object} this
         */
        ofScan: function(outerScan){
            if(!this) { return; }
    
            var $contexte = this;
    
            of.head.scanSels.forEach(function(scanSel){
    
                var $toScan = $contexte.find( scanSel.sel ),
                    nbElts;
                ...
                $toScan.each(function(){
                    var $o = $(this),
                        doneScans = $o.data('doneScans.of') || [];
    
                    if( -1 == $.inArray(scanSel.sel, doneScans ) ) {
                        scanSel.fn.call(this, $contexte); // en param, la callback aura le context de scan (this);
                        doneScans.push(scanSel.sel);
                        $o.data('doneScans.of', doneScans);
                    }
        ...
        onScan: function(sel,fn) {
            of.head.scanSels.push({sel:sel,fn:fn});
        }
    });


    // -~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~
    // Config auto completion commune
    ...
    // -~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~
    of.head.accore = {
        resultMap: function(loc) {
            return { value: loc.libelle, data: {
                'insee': loc.insee,
                'libelle': loc.libelle,
                'url': loc.url
            } };
        ...
        submitAction: function($e) {
            var a = $e.data('autocomplete');
            if(a && a.currentValue) {
                for (var i = a.suggestions.length - 1; i >= 0; i--) {
                    if(a.suggestions[i].value == a.currentValue) {
                        window.location = a.suggestions[i].data.url;
                    }
                }
            }
        },
        selectAction: function(suggestion) {
    ...
    of.head.asyncLoadOf = function() {

        of.head.$doc.ofScan(); // first scan sur tout le doc

        $('html').addClass('ofready').trigger('ofready');
    };

})(jQuery, window.of); // End Of of.jquery-noConflict-start.js
